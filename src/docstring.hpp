...
namespace xfift {
    static inline std::string get_docstring(const std::string& word) {
        static std::unordered_map<std::string, std::string> docs = {
            ...
            {"XCHG", "01 — XCHG s1, also known as SWAP.\n0i — XCHG s(i) or XCHG s0,s(i), interchanges the top of the stack with s(i), 1≤ i≤ 15.\n0i — XCHG s(i) or XCHG s0,s(i), interchanges the top of the stack with s(i), 1≤ i≤ 15.\n10ij — XCHG s(i),s(j), 1≤ i<j≤15, interchanges s(i) with s(j).\n11ii — XCHG s0,s(ii)\\nWith 0≤ ii≤255.\n1i — XCHG s1,s(i), 2≤ i≤ 15."},
            {"SWAP", "01 — XCHG s1, also known as SWAP."},
            {"PUSH", "2i — PUSH s(i), 0≤ i≤ 15, pushes a copy of the old s(i) into the stack.\n20 — PUSH s0, also known as DUP.\n21 — PUSH s1, also known as OVER.\n56ii — PUSH s(ii) for 0≤ ii≤ 255.\nED4i — PUSH c(i) or PUSHCTR c(i) ( — x ), pushes the current value of control register c(i). If the control register is not supported in the current codepage, or if it does not have a value, an exception is triggered.\nED44 — PUSH c4 or PUSHROOT, pushes the “global data root” cell reference, thus enabling access to persistent smart-contract data."},
            {"DUP", "20 — PUSH s0, also known as DUP."},
            {"OVER", "21 — PUSH s1, also known as OVER."},
            {"POP", "3i — POP s(i), 0≤ i≤ 15, pops the old top-of-stack value into the old s(i).\n30 — POP s0, also known as DROP, discards the top-of-stack value.\n31 — POP s1, also known as NIP.\n57ii — POP s(ii) for 0≤ ii≤ 255.\nED5i — POP c(i) or POPCTR c(i) ( x — ), pops a value x from the stack and stores it into control register c(i), if supported in the current codepage. Notice that if a control register accepts only values of a specific type, a type-checking exception may occur.\nED54 — POP c4 or POPROOT, sets the “global data root” cell reference, thus allowing modification of persistent smart-contract data."},
            {"DROP", "30 — POP s0, also known as DROP, discards the top-of-stack value."},
            {"NIP", "31 — POP s1, also known as NIP."},
            {"XCHG3", "4ijk — XCHG3 s(i),s(j),s(k), equivalent to XCHG s2,s(i); XCHG s1, s(j); XCHG s0,s(k), with 0≤ i,j,k≤ 15.\n540ijk — XCHG3 s(i),s(j),s(k) (long form)."},
            {"XCHG2", "50ij — XCHG2 s(i),s(j), equivalent to XCHG s1,s(i); XCHG s(j)."},
            {"XCPU", "51ij — XCPU s(i),s(j), equivalent to XCHG s(i); PUSH s(j)."},
            {"PUXC", "52ij — PUXC s(i),s(j-1), equivalent to PUSH s(i); SWAP; XCHG s(j)."},
            {"PUSH2", "53ij — PUSH2 s(i),s(j), equivalent to PUSH s(i); PUSH s(j+1)."},
            {"XC2PU", "541ijk — XC2PU s(i),s(j),s(k), equivalent to XCHG2 s(i),s(j); PUSH s(k)."},
            {"XCPUXC", "542ijk — XCPUXC s(i),s(j),s(k-1), equivalent to XCHG s1,s(i); PUXC s(j),s(k-1)."},
            {"XCPU2", "543ijk — XCPU2 s(i),s(j),s(k), equivalent to XCHG s(i); PUSH2 s(j), s(k)."},
            {"PUXC2", "544ijk — PUXC2 s(i),s(j-1),s(k-1), equivalent to PUSH s(i); XCHG s2; XCHG2 s(j),s(k)."},
            {"PUXCPU", "545ijk — PUXCPU s(i),s(j-1),s(k-1), equivalent to PUXC s(i),s(j-1); PUSH s(k)."},
            {"PU2XC", "546ijk — PU2XC s(i),s(j-1),s(k-2), equivalent to PUSH s(i); SWAP; PUXC s(j),s(k-1)."},
            {"PUSH3", "547ijk — PUSH3 s(i),s(j),s(k), equivalent to PUSH s(i); PUSH2 s(j+1),s(k+1)."},
            {"BLKSWAP", "55ij — BLKSWAP i+1,j+1, permutes two blocks s(j+i+1) … and s(j) …, for 0≤ i,j≤15. Equivalent to REVERSE i+1,j+1; REVERSE j+1,0; REVERSE i+j+2,0."},
            {"ROT2", "5513 — ROT2 or 2ROT ( a b c d e f — c d e f a b )\\nRotates the three topmost pairs of stack entries."},
            {"2ROT", "5513 — ROT2 or 2ROT ( a b c d e f — c d e f a b )\\nRotates the three topmost pairs of stack entries."},
            {"ROLL", "550i — ROLL i+1, rotates the top i+1 stack entries. Equivalent to BLKSWAP 1,i+1."},
            {"ROLLREV", "55i0 — ROLLREV i+1 or -ROLL i+1, rotates the top i+1 stack entries in the other direction. Equivalent to BLKSWAP i+1,1."},
            {"-ROLL", "55i0 — ROLLREV i+1 or -ROLL i+1, rotates the top i+1 stack entries in the other direction. Equivalent to BLKSWAP i+1,1."},
            ...
            {"-ROT", "59 — ROTREV or -ROT ( a b c — c a b )\\nEquivalent to BLKSWAP 2,1 or to XCHG2 s2,s2."},
            {"SWAP2", "5A — SWAP2 or 2SWAP ( a b c d — c d a b )\\nEquivalent to BLKSWAP 2,2 or to XCHG2 s3,s2."},
            {"2SWAP", "5A — SWAP2 or 2SWAP ( a b c d — c d a b )\\nEquivalent to BLKSWAP 2,2 or to XCHG2 s3,s2."},
            {"DROP2", "5B — DROP2 or 2DROP ( a b — )\\nEquivalent to DROP; DROP."},
            {"2DROP", "5B — DROP2 or 2DROP ( a b — )\\nEquivalent to DROP; DROP."},
            {"DUP2", "5C — DUP2 or 2DUP ( a b — a b a b )\\nEquivalent to PUSH2 s1,s0."},
            {"2DUP", "5C — DUP2 or 2DUP ( a b — a b a b )\\nEquivalent to PUSH2 s1,s0."},
            {"OVER2", "5D — OVER2 or 2OVER ( a b c d — a b c d a b )\\nEquivalent to PUSH2 s3,s2."},
            {"2OVER", "5D — OVER2 or 2OVER ( a b c d — a b c d a b )\\nEquivalent to PUSH2 s3,s2."},
            {"REVERSE", "5Eij — REVERSE i+2,j, reverses the order of s(j+i+1) … for 0≤ i,j≤ 15; equivalent to a sequence of ⌊i/2⌋ +1 XCHG s."},
            {"BLKDROP", "5F0i — BLKDROP i, equivalent to DROP performed i times."},
            {"BLKPUSH", "5Fij — BLKPUSH i,j, equivalent to PUSH s(j) performed i times, 1≤ i≤ 15, 0≤ j≤ 15."},
            {"PICK", "60 — PICK or PUSHX, pops integer i from the stack, then performs PUSH s(i)."},
            {"PUSHX", "60 — PICK or PUSHX, pops integer i from the stack, then performs PUSH s(i)."},
            {"ROLLX", "61 — ROLLX, pops integer i from the stack, then performs BLKSWAP 1,i."},
            {"-ROLLX", "62 — -ROLLX or ROLLREVX, pops integer i from the stack, then performs BLKSWAP i,1."},
            {"ROLLREVX", "62 — -ROLLX or ROLLREVX, pops integer i from the stack, then performs BLKSWAP i,1."},
            {"BLKSWX", "63 — BLKSWX, pops integers i, j from the stack, then performs BLKSWAP i,j."},
            {"REVX", "64 — REVX, pops integers i, j from the stack, then performs REVERSE i,j."},
            {"DROPX", "65 — DROPX, pops integer i from the stack, then performs BLKDROP i."},
            ...
            {"COMMA", "6F8C — TPUSH or COMMA ( t x — t' )\\nAppends a value x to a Tuple t=(x₁,…,xₙ), but only if the resulting Tuple t'=(x₁,…,xₙ,x) is of length at most 255. Otherwise throws a type check exception."},
            ...
            {"INDEX2", "6FBij — INDEX2 i,j ( t — x )\\nRecovers x=(t_{i+1})_{j+1} for 0≤ i,j≤ 3. Equivalent to INDEX i; INDEX j."},
            {"CADR", "6FB4 — CADR ( t — x )\\nRecovers x=(t_2)₁."},
            {"CDDR", "6FB5 — CDDR ( t — x )\\nRecovers x=(t_2)_2."},
            {"INDEX3", "6FE_ijk — INDEX3 i,j,k ( t — x )\\nRecovers x= ((t_{i+1})_{j+1} )_{k+1} for 0≤ i,j,k≤3. Equivalent to INDEX2 i,j; INDEX k."},
            {"CADDR", "6FD4 — CADDR ( t — x )\\nRecovers x= ((t_2)_2 )₁."},
            {"CDDDR", "6FD5 — CDDDR ( t — x )\\nRecovers x= ((t_2)_2 )_2."},
            {"PUSHINT", "7i — PUSHINT x with -5≤ x≤ 10, pushes integer x into the stack; here i equals four lower-order bits of x (i.e., i=x mod 16 ).\n71 — ONE or PUSHINT 1.\n72 — TWO or PUSHINT 2.\n7A — TEN or PUSHINT 10.\n7F — TRUE or PUSHINT -1.\n80xx — PUSHINT xx with -128≤ xx≤127.\n81xxxx — PUSHINT xxxx with -2^{15}≤ xxxx<2^{15} a signed 16-bit big-endian integer.\n81FC18 — PUSHINT -1000.\n82lxxx — PUSHINT xxx, where 5-bit 0≤ l≤30 determines the length n=8l+19 of signed big-endian integer xxx. The total length of this instruction is l+4 bytes or n+13=8l+32 bits.\n821005F5E100 — PUSHINT 10^8."},
            ...
            {"PUSHPOW2", "83xx — PUSHPOW2 xx+1, (quietly) pushes 2^{xx+1} for 0≤ xx≤255."},
            {"PUSHNAN", "83FF — PUSHNAN, pushes a NaN."},
            {"PUSHPOW2DEC", "84xx — PUSHPOW2DEC xx+1, pushes 2^{xx+1}-1 for 0≤ xx≤ 255."},
            {"PUSHNEGPOW2", "85xx — PUSHNEGPOW2 xx+1, pushes -2^{xx+1} for 0≤ xx≤ 255."},
            ...
            {"DEC", "A5 — DEC ( x — x-1 )\\nEquivalent to ADDCONST -1."},
            {"ADDCONST", "A6cc — ADDCONST cc ( x — x+cc ), -128≤ cc≤127."},
            {"MULCONST", "A7cc — MULCONST cc ( x — x⋅ cc ), -128≤ cc≤127."},
            {"MUL", "A8 — MUL ( x y — xy )."},
            ...
            {"EQINT", "C0yy — EQINT yy ( x — x=yy ) for -2^7≤ yy<2^7."},
            {"ISZERO", "C000 — ISZERO, checks whether an integer is zero. Corresponds to Forth's 0=."},
            {"LESSINT", "C1yy — LESSINT yy ( x — x<yy ) for -2^7≤ yy<2^7."},
            ...
            {"GTINT", "C2yy — GTINT yy ( x — x>yy ) for -2^7≤ yy<2^7."},
            ...
            {"NEQINT", "C3yy — NEQINT yy ( x — x≠ yy ) for -2^7≤ yy<2^7."},
            ...
            {"STIR", "CF0Acc — STIR cc+1 ( b x — b' )\\nEquivalent to SWAP; STI cc+1."},
            {"STUR", "CF0Bcc — STUR cc+1 ( b x — b' )\\nEquivalent to SWAP; STU cc+1."},
            {"STIQ", "CF0Ccc — STIQ cc+1 ( x b — x b f or b' 0 )."},
            {"STUQ", "CF0Dcc — STUQ cc+1 ( x b — x b f or b' 0 )."},
            {"STIRQ", "CF0Ecc — STIRQ cc+1 ( b x — b x f or b' 0 )."},
            {"STURQ", "CF0Fcc — STURQ cc+1 ( b x — b x f or b' 0 )."},
            ...
            {"STREF2CONST", "CF21 — STREF2CONST, equivalent to STREFCONST; STREFCONST."},
            ...
            {"STSLICECONST", "CFC0_xysss — STSLICECONST sss ( b — b' )\\nStores a constant subslice sss consisting of 0≤ x≤ 3 references and up to 8y+1 data bits, with 0≤ y≤ 7. Completion bit is assumed.\nCF81 — STSLICECONST `0' or STZERO ( b — b' )\\nStores one binary zero.\nCF83 — STSLICECONST `1' or STONE ( b — b' )\\nStores one binary one."},
            {"STZERO", "CF81 — STSLICECONST `0' or STZERO ( b — b' )\\nStores one binary zero."},
            {"STONE", "CF83 — STSLICECONST `1' or STONE ( b — b' )\\nStores one binary one."},
            {"STREF3CONST", "CFE2 — STREF3CONST."},
            ...
            {"PLDI", "D70Acc — PLDI cc+1 ( s — x )\\nPreloads a signed cc+1 -bit integer from Slice s."},
            {"PLDU", "D70Bcc — PLDU cc+1 ( s — x )\\nPreloads an unsigned cc+1 -bit integer from s."},
            {"LDIQ", "D70Ccc — LDIQ cc+1 ( s — x s' -1 or s 0 )\\nA quiet version of LDI."},
            {"LDUQ", "D70Dcc — LDUQ cc+1 ( s — x s' -1 or s 0 )\\nA quiet version of LDU."},
            {"PLDIQ", "D70Ecc — PLDIQ cc+1 ( s — x -1 or 0 )\\nA quiet version of PLDI."},
            {"PLDUQ", "D70Fcc — PLDUQ cc+1 ( s — x -1 or 0 )\\nA quiet version of PLDU."},
            {"PLDUZ", "D714_c — PLDUZ 32(c+1) ( s — s x ), preloads the first 32(c+1) bits of Slice s into an unsigned integer x, for 0≤ c≤ 7. If s is shorter than necessary, missing bits are assumed to be zero. This operation is intended to be used along with IFBITJMP and similar instructions."},
            ...
            {"PLDSLICE", "D71Dcc — PLDSLICE cc+1 ( s — s” )\\nReturns the first 0<cc+1≤ 256 bits of s as s”."},
            {"LDSLICEQ", "D71Ecc — LDSLICEQ cc+1 ( s — s” s' -1 or s 0 )\\nA quiet version of LDSLICE."},
            {"PLDSLICEQ", "D71Fcc — PLDSLICEQ cc+1 ( s — s” -1 or 0 )\\nA quiet version of PLDSLICE."},
            ...
            {"BLESSARGS", "EErn — BLESSARGS r,n ( x₁ … x_r s — c )\\nWhere 0≤ r≤ 15, -1≤ n≤ 14, equivalent to BLESS; SETCONTARGS r,n. The value of n is represented inside the instruction by the 4-bit integer n mod16.\nEE0n — BLESSNUMARGS n or BLESSARGS 0,n ( s — c )\\nAlso transforms a Slice s into a Continuation c, but sets c.nargs to 0≤ n≤ 14.\nEErn — BLESSARGS r,n ( x₁ … x_r s — c )\\nDescribed in sp:cont.stk.manip."},
            {"BLESSNUMARGS", "EE0n — BLESSNUMARGS n or BLESSARGS 0,n ( s — c )\\nAlso transforms a Slice s into a Continuation c, but sets c.nargs to 0≤ n≤ 14."},
            {"PUSHCTR", "ED4i — PUSH c(i) or PUSHCTR c(i) ( — x ), pushes the current value of control register c(i). If the control register is not supported in the current codepage, or if it does not have a value, an exception is triggered."},
            {"PUSHROOT", "ED44 — PUSH c4 or PUSHROOT, pushes the “global data root” cell reference, thus enabling access to persistent smart-contract data."},
            {"POPCTR", "ED5i — POP c(i) or POPCTR c(i) ( x — ), pops a value x from the stack and stores it into control register c(i), if supported in the current codepage. Notice that if a control register accepts only values of a specific type, a type-checking exception may occur."},
            {"POPROOT", "ED54 — POP c4 or POPROOT, sets the “global data root” cell reference, thus allowing modification of persistent smart-contract data."},
            {"SETCONT", "ED6i — SETCONT c(i) or SETCONTCTR c(i) ( x c — c' ), stores x into the savelist of continuation c as c(i), and returns the resulting continuation c'. Almost all operations with continuations may be expressed in terms of SETCONTCTR, POPCTR, and PUSHCTR."},
            {"SETCONTCTR", "ED6i — SETCONT c(i) or SETCONTCTR c(i) ( x c — c' ), stores x into the savelist of continuation c as c(i), and returns the resulting continuation c'. Almost all operations with continuations may be expressed in terms of SETCONTCTR, POPCTR, and PUSHCTR."},
            {"SETRETCTR", "ED7i — SETRETCTR c(i) ( x — ), equivalent to PUSH c0; SETCONTCTR c(i); POP c0."},
            {"SETALTCTR", "ED8i — SETALTCTR c(i) ( x — ), equivalent to PUSH c1; SETCONTCTR c(i); POP c0."},
            {"POPSAVE", "ED9i — POPSAVE c(i) or POPCTRSAVE c(i) ( x —), similar to POP c(i), but also saves the old value of c(i) into continuation c0. Equivalent (up to exceptions) to SAVECTR c(i); POP c(i)."},
            {"POPCTRSAVE", "ED9i — POPSAVE c(i) or POPCTRSAVE c(i) ( x —), similar to POP c(i), but also saves the old value of c(i) into continuation c0. Equivalent (up to exceptions) to SAVECTR c(i); POP c(i)."},
            {"SAVE", "EDAi — SAVE c(i) or SAVECTR c(i) ( — ), saves the current value of c(i) into the savelist of continuation c0. If an entry for c(i) is already present in the savelist of c0, nothing is done. Equivalent to PUSH c(i); SETRETCTR c(i)."},
            {"SAVECTR", "EDAi — SAVE c(i) or SAVECTR c(i) ( — ), saves the current value of c(i) into the savelist of continuation c0. If an entry for c(i) is already present in the savelist of c0, nothing is done. Equivalent to PUSH c(i); SETRETCTR c(i)."},
            {"SAVEALT", "EDBi — SAVEALT c(i) or SAVEALTCTR c(i) ( — ), similar to SAVE c(i), but saves the current value of c(i) into the savelist of c1, not c0."},
            {"SAVEALTCTR", "EDBi — SAVEALT c(i) or SAVEALTCTR c(i) ( — ), similar to SAVE c(i), but saves the current value of c(i) into the savelist of c1, not c0."},
            {"SAVEBOTH", "EDCi — SAVEBOTH c(i) or SAVEBOTHCTR c(i) ( — ), equivalent to DUP; SAVE c(i); SAVEALT c(i)."},
            {"SAVEBOTHCTR", "EDCi — SAVEBOTH c(i) or SAVEBOTHCTR c(i) ( — ), equivalent to DUP; SAVE c(i); SAVEALT c(i)."},
            ...
            {"BOOLAND", "EDF0 — COMPOS or BOOLAND ( c c' — c” )\\nComputes the composition c◦₀c', which has the meaning of “perform c, and, if successful, perform c' ” (if c is a boolean circuit) or simply “perform c, then c' ”. Equivalent to SWAP; SETCONT c0."},
            ...
            {"BOOLOR", "EDF1 — COMPOSALT or BOOLOR ( c c' — c” )\\nComputes the alternative composition c◦₁ c', which has the meaning of “perform c, and, if not successful, perform c' ” (if c is a boolean circuit). Equivalent to SWAP; SETCONT c1."},
            ...
            {"STOPTREF", "F400 — STDICT or STOPTREF ( D b — b' )\\nStores dictionary D into Builder b, returing the resulting Builder b'. In other words, if D is a cell, performs STONE and STREF; if D is Null, performs NIP and STZERO; otherwise throws a type checking exception."},
            {"SKIPDICT", "F401 — SKIPDICT or SKIPOPTREF ( s — s' )\\nEquivalent to LDDICT; NIP."},
            {"SKIPOPTREF", "F401 — SKIPDICT or SKIPOPTREF ( s — s' )\\nEquivalent to LDDICT; NIP."},
            ...
            {"PFXDICTCONSTGETJMP", "F4AEₙ — PFXDICTCONSTGETJMP n or PFXDICTSWITCH n ( s — s' s” or s )\\nCombines DICTPUSHCONST n for 0≤ n≤ 1023 with PFXDICTGETJMP."},
            {"PFXDICTSWITCH", "F4AEₙ — PFXDICTCONSTGETJMP n or PFXDICTSWITCH n ( s — s' s” or s )\\nCombines DICTPUSHCONST n for 0≤ n≤ 1023 with PFXDICTGETJMP."},
            ...
            {"DEBUG", "FEnn — DEBUG nn, for 0≤ nn<240, is a two-byte NOP."},
            {"DEBUGSTR", "FEFnssss — DEBUGSTR ssss, for 0≤ n<16, is an (n+3) -byte NOP, with the (n+1) -byte “contents string” ssss skipped as well."},
            {"DUMPSTK", "FE00 — DUMPSTK, dumps the stack (at most the top 255 values) and shows the total stack depth."},
            {"DUMPSTKTOP", "FE0n — DUMPSTKTOP n, 1≤ n<15, dumps the top n values from the stack, starting from the deepest of them. If there are d<n values available, dumps only d values."},
            ...
            {"DUMP", "FE2n — DUMP s(n), 0≤ n<15, dumps s (n)."},
            {"PRINT", "FE3n — PRINT s(n), 0≤ n<15, concatenates the text representation of s (n) (without any leading or trailing spaces or carriage returns) to a text buffer which will be output before the output of any other debug operation."},
            {"DUMPTOSFMT", "FEFnssss — DUMPTOSFMT ssss, dumps s0 formatted according to the (n+1) -byte string ssss. This string might contain (a prefix of) the name of a TL-B type supported by the debugger. If the string begins with a zero byte, simply outputs it (without the first byte) into the debug log. If the string begins with a byte equal to one, concatenates it to a buffer, which will be output before the output of any other debug operation (effectively outputs a string without a carriage return)."},
            {"LOGSTR", "FEFn00ssss — LOGSTR ssss, string ssss is n bytes long."},
            {"LOGFLUSH", "FEF000 — LOGFLUSH, flushes all pending debug output from the buffer into the debug log."},
            {"PRINTSTR", "FEFn01ssss — PRINTSTR ssss, string ssss is n bytes long."},
            {"SETCP", "FFnn — SETCP nn, selects TVM codepage 0≤ nn<240. If the codepage is not supported, throws an invalid opcode exception.\nFFFz — SETCP z-16, selects TVM codepage z-16 for 1≤ z≤ 15. Negative codepages -13…-1 are reserved for restricted versions of TVM needed to validate runs of TVM in other codepages as explained in sp:cp.minusone. Negative codepage -14 is reserved for experimental codepages, not necessarily compatible between different TVM implementations, and should be disabled in the production versions of TVM."},
            {"SETCP0", "FF00 — SETCP0, selects TVM (test) codepage zero as described in this document."},
