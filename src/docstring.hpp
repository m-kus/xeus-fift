...
namespace xfift {
    static inline std::string get_docstring(const std::string& word) {
        static std::unordered_map<std::string, std::string> docs = {
            ...
            {"\"〈string〉\"", "\"〈string〉\" ( -- S ), pushes a String literal into the stack."},
            ...
            {"(〈string/〉)", "(〈string/〉) ( -- … ), looks up the word $〈string/〉 during execution time and executes its current definition. Typically used to access the current values of command-line arguments, e.g., $(2) is essentially equivalent to @' $2."},
            ...
            {"$n", "$n ( -- S ), pushes the n -th command-line argument as a String S. For instance, $0 pushes the name of the script being executed, $1 the first command line argument, and so on. Defined only when the Fift interpreter is invoked in script mode (with the -s command line argument)."},
            ...
            {"'〈word-name〉", "'〈word-name〉 ( -- e ), returns the execution token equal to the current (compile-time) definition of 〈word-name〉. If the specified word is not found, throws an exception."},
            {"'nop", "'nop ( -- e ), pushes the default definition of nop ---an execution token that does nothing when executed."},
            {"(')〈word-name〉", "(')〈word-name〉 ( -- e ), similar to ', but returns the definition of the specified word at execution time, performing a dictionary lookup each time it is invoked. May be used to recover the current values of constants inside word definitions and other blocks by using the phrase (')〈word-name〉execute."},
            ...
            {"*>>c", "*>>c ( x y z -- q ), similar to */c, but with division replaced with a right shift. Computes q:=lceil xy/2^zrceil for 0≤z≤256. Equivalent to 1<< */c."},
            {"*>>r", "*>>r ( x y z -- q ), similar to */r, but with division replaced with a right shift. Computes q:=⌊xy/2^z+1/2⌋ for 0≤z≤256. Equivalent to 1<< */r."},
            ...
            {"+\"〈string〉\"", "+\"〈string〉\" ( S -- S' ), concatenates String S with a string literal. Equivalent to \"〈string〉\" $+."},
            ...
            {".\"〈string〉\"", ".\"〈string〉\" ( -- ), prints a constant string into the standard output."},
            ...
            {"/*〈multiline-comment〉*/", "/*〈multiline-comment〉*/ ( -- ), skips a multi-line comment delimited by word “ */ ” (followed by a blank or an end-of-line character)."},
            {"//〈comment-to-eol〉", "//〈comment-to-eol〉 ( -- ), skips a single-line comment until the end of the current line."},
            ...
            {"2=:〈word-name〉", "2=:〈word-name〉 ( x y -- ), an active variant of 2constant: defines a new ordinary word 〈word-name〉 that would push the given values x and y when invoked."},
            ...
            {":〈word-name〉", ":〈word-name〉 ( e -- ), defines a new ordinary word 〈word-name〉 in the dictionary using WordDef e as its definition. If the specified word is already present in the dictionary, it is tacitly redefined."},
            {"::〈word-name〉", "::〈word-name〉 ( e -- ), defines a new active word 〈word-name〉 in the dictionary using WordDef e as its definition. If the specified word is already present in the dictionary, it is tacitly redefined."},
            {"::_〈word-name〉", "::_〈word-name〉 ( e -- ), defines a new active prefix word 〈word-name〉 in the dictionary using WordDef e as its definition. If the specified word is already present in the dictionary, it is tacitly redefined."},
            {":_〈word-name〉", ":_〈word-name〉 ( e -- ), defines a new ordinary prefix word 〈word-name〉 in the dictionary using WordDef e as its definition. If the specified word is already present in the dictionary, it is tacitly redefined."},
            ...
            {"<</c", "<</c ( x y z -- q ), computes q:=lceil 2^zx/yrceil for 0≤z≤256 producing a 513-bit intermediate result, similarly to */c. Equivalent to 1<< swap */c."},
            {"<</r", "<</r ( x y z -- q ), computes q:=⌊2^zx/y+1/2⌋ for 0≤z≤256 producing a 513-bit intermediate result, similarly to */r. Equivalent to 1<< swap */r."},
            ...
            {"=:〈word-name〉", "=:〈word-name〉 ( x -- ), an active variant of constant: defines a new ordinary word 〈word-name〉 that would push the given value x when invoked."},
            ...
            {">>c", ">>c ( x y -- q:=lceil xcdot 2^{-y}rceil ), computes the ceiling-rounded quotient q of x by 2^y for 0≤y≤256. Equivalent to 1<< /c."},
            {">>r", ">>r ( x y -- q:=⌊xcdot 2^{-y}+1/2⌋ ), computes the nearest-integer-rounded quotient q of x by 2^y for 0≤y≤256. Equivalent to 1<< /r."},
            ...
            {"@'〈word-name〉", "@'〈word-name〉 ( -- e ), recovers the definition of the specified word at execution time, performing a dictionary lookup each time it is invoked, and then executes this definition. May be used to recover current values of constants inside word definitions and other blocks by using the phrase @'〈word-name〉, equivalent to (')〈word-name〉execute."},
            ...
            {"B{〈{hex-digits}〉}", "B{〈{hex-digits}〉} ( -- B ), pushes a Bytes literal containing data represented by an even number of hexadecimal digits."},
            ...
            {"compile〈word-name〉", "compile〈word-name〉 ( -- ), compiles 〈word-name〉 as if it were an ordinary word, even if it is active. Essentially equivalent to '〈word-name〉execute."},
            ...
            {"`〈word/〉", "`〈word/〉 ( -- a ), introduces an Atom literal, equal to the only Atom with the name equal to 〈word/〉. Equivalent to \"〈word/〉\" atom."},
            {"abort", "abort ( S -- ), throws an exception with an error message taken from String S."},
            {"abort\"〈message〉\"", "abort\"〈message〉\" ( x -- ), throws an exception with the error message 〈message〉 if the Integer x is non-zero."},
            ...
            {"b{〈binary-data〉}", "b{〈binary-data〉} ( -- s ), creates a Slice s that contains no references and up to 1023 data bits specified in 〈binary-data〉, which must be a string consisting only of the characters ` 0 ' and ` 1 '."},
            ...
            {"char〈string〉", "char〈string〉 ( -- x ), pushes an Integer with the Unicode codepoint of the first character of 〈string〉. For instance, char * is equivalent to 42."},
            ...
            {"gasrunvm", "gasrunvm ( … s c z -- … x c' z' ), a gas-aware version of runvm. 1.4 TVM ) with Cell c. Then starts the new TVM instance with the gas limit set to z. The actually consumed gas z' is returned at the top of the final Fift stack, and the final value of c4 is returned immediately below the top of the final Fift stack as another Cell c'."},
            ...
            {"hashB", "hashB ( c -- B ), computes the Sha -based representation hash of Cell c (cf. 3.1 TVM ), which unambiguously defines c and all its descendants (provided there are no collisions for Sha ). The result is returned as a Bytes value consisting of exactly 32 bytes."},
